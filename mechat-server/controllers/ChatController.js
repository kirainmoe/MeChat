...
class ChatController {
    ...
    async getMessageList(req, response) {
        ...
        const fromThisUser = await this.mdb
            .find({
                from: uid,
                target: "friends"
            })
        ...
        const toThisUser = await this.mdb
            .find({
                to: uid,
                target: "friends"
            })
        ...
        for (const id in all) {
            const records = await this.mdb
                .find({
                    $or: [{ $and: [{ from: uid, to: id }] }, { $and: [{ from: id, to: uid }] }]
                })
                .sort({ timestamp: -1 })
                .limit(20);
            let tmp = [];
            for (const i in records) {
                ...
                if (!record.read && record.from == 0) {
                    all[id].read = false;
                    await this.mdb.findOneAndUpdate(
